...
FileResult = namedtuple("FileResult", "num_stories " "num_failed_stories ")
...
def process_failed_story(failed_story):
    ...
    for line in failed_story.splitlines():
        ...
        if re.search("<!--.*-->", line):
            error_prediction = (
                re.search("<!--(.*)-->", line).group(1).strip().split(":")
            )
            error_message = ""
            ...
            if is_intent:
                error_message += "  Intent failed: Predicted intent" + (
                    "{} for message '{}'".format(
                        error_prediction[1], error_prediction[2].strip()
                    )
                ...
                error_message += (
                    "      Utter failed: Predicted utter" + error_prediction[1]
                )
...
def run_evaluation(
    ...
    story_evaluation, _ = collect_story_predictions(
        completed_trackers, _agent, fail_on_prediction_errors, use_e2e
    )
    _failed_stories = story_evaluation.failed_stories

    _num_stories = len(completed_trackers)
    _file_result = FileResult(
        num_stories=_num_stories, num_failed_stories=len(_failed_stories)
    )
    ...
    if len(_failed_stories) == 0:
        success_message = "The stories have passed for file '{}'!!".format(
            file_to_evaluate
        )
...
if __name__ == "__main__":
    ...
    if os.path.isdir(stories_path):
        ...
        stories_files = [
            f for f in listdir(stories_path) if isfile(join(stories_path, f))
        ]
